...
| Commit | The session's own commits on the branch; conventional style per CLAUDE.md. **Qualify the task ID with its work item when more than one work item is in flight** — `feat(<WI>/<T>): ...` rather than `feat(<T>): ...`. Task IDs restart at `T-001` in every task list, so an unqualified subject is ambiguous and `next-step.py` will refuse to credit it (see "Commit evidence" below). |
...
implemented and **skips** the work, while a missed credit only costs a redundant pass. Repos
with one work item in flight are unaffected — unqualified subjects still count.
